...
def distillation_loss(student_logits, teacher_logits, temperature=1.0, reduction='batchmean'):
    ...
    # KL 散度是如何计算的？
    # 假设 B == 1，这更好理解，B != 1 同理，计算过程是极其严谨的两步走，先在**词表维度（Vocab）求和，再在序列维度（Sequence）**处理。首先 KL 散度的公式为 KL(P \| Q) = \sum_{v=1}^{V} P_v \cdot \log \frac{P_v}{Q_v}。
    # 首先我们需要计算每个位置的“局部偏差”。假设这个样本长度为 R，词表大小为 V。在执行 F.kl_div 时，内部会先产生一个形状为 [R, V] 的中间矩阵。这个矩阵的每一行，都是针对当前位置生成的词，计算学生和老师在全词表上的差异。
    # 然后从“面”压缩到“点” (词表维度求和)，按照 KL 散度的定义，我们必须把词表这一维压掉。此时，我们得到了一个长度为 R 的向量。这个向量里的每一个值，就是该样本在每一个 Token 位置上的 KL 散度。之后这个 R 的向量如何处理取决于 reduction 参数，可以是 sum，也可以是 mean。
    # ⭐️⭐️⭐️⭐️⭐️ 所以在本质上，KL 散度只是在评估某个位置上，两个模型输出 token 的概率的分布差异。
